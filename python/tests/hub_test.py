...
def test_empty_messages():
    ...
    h1 = Hub(timeout=TIMEOUT)
    h2 = Hub(timeout=TIMEOUT)
    assert h1.getAll('Test') == []
    assert h2.getAll('Test') == []
    assert h1.getAll('Test2') == []
    assert h2.getAll('Test2') == []
...
def test_oneway_messages():
    ...
    h1.connect('Test', '127.0.0.1', h2.port)
    s(0.1)
    h1.writeToName('Test', 'Test', 'test of port 1')
    s(0.05)
    assert h1.getAll('Test') == []
    assert h2.getAll('Test') == ['test of port 1']
    assert h1.getAll('Test2') == []
    assert h2.getAll('Test2') == []
    h1.writeToName('Test', 'Test2', 'test of port 2')
    s(0.05)
    assert h1.getAll('Test') == []
    assert h2.getAll('Test') == ['test of port 1']
    assert h1.getAll('Test2') == []
    assert h2.getAll('Test2') == ['test of port 2']
    h1.close()
    h2.close()
...
def test_bidirectional_messages():
    ...
    assert h2.getAll('Test2') == []
    h1.writeToName('Test', 'Test', 'test of port 1')
    h2.writeToName('Test', 'Test', 'test of port 1')
    s(0.05)
    assert h1.getAll('Test') == ['test of port 1']
    assert h2.getAll('Test') == ['test of port 1']
    assert h1.getAll('Test2') == []
    assert h2.getAll('Test2') == []
    h1.writeToName('Test', 'Test2', 'test of port 2')
    h2.writeToName('Test', 'Test2', 'test of port 2')
    s(0.05)
    assert h1.getAll('Test') == ['test of port 1']
    assert h2.getAll('Test') == ['test of port 1']
    assert h1.getAll('Test2') == ['test of port 2']
    assert h2.getAll('Test2') == ['test of port 2']
    h1.close()
    h2.close()
...
def test_high_speed(num=10, bidirectional=False, sleep=0.1):
    h1, h2 = initialize()
    for i in range(num):
        h1.writeToName(CHANNEL, 'test{}'.format(i), text)
        if bidirectional:
            h2.writeToName(CHANNEL, 'test{}'.format(i), text)
    s(sleep)
    assert len(h1.transports) == len(h2.transports)
    for i in range(num):
        if bidirectional:
            assert h1.getAll('test{}'.format(i)) == [text]
        assert h2.getAll('test{}'.format(i)) == [text]
    h1.close()
    h2.close()
...
def multi_transports(num_conn=10, messages=10, bidirectional=False):
    ...
    for i in range(messages):
        h1.writeAll('Test{}'.format(i), text)
        if bidirectional:
            h2.writeAll('Test{}'.format(i), text)
    s()
    for i in range(messages):
        if bidirectional:
            assert h1.getAll('Test{}'.format(i)) == [text] * num_conn
        assert h2.getAll('Test{}'.format(i)) == [text] * num_conn
    h1.close()
    h2.close()
...
def stress_test(stress=5, messages=1000):
    ...
    s()
    for i in range(messages):
        assert h1.getAll('Test{}'.format(i)) == [text] * stress
        assert h2.getAll('Test{}'.format(i)) == [text] * stress
    h1.close()
    h2.close()
...
def multi_hub_stress_test(stress=5, messages=1000):
    ...
    for h in hubs:
        for i in range(messages):
            assert h.getAll('Test{}'.format(i)) == [text] * 2 * stress
